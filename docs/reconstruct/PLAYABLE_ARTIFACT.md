...
**[DECISION]** Lane E2 owns scoring (`src/lib/eval/**`). Lane H calls `evaluateVideoQa` / `videoQaInputFromReconstructE2e` / `videoQaReportToJson` only. Encode-first with `frames:[]` is INCOMPLETE (`awaiting decoded_frames`); `blockingArtifactProducer` is always false. **H owns MP4 decode:** node/ffmpeg (PR #132) and browser WebCodecs (`decodeMp4Browser.ts`). Injected `decodedFrames` score with `frames>0`. Live Export samples at most 8 frames of the 72-frame gate (not the 8-frame UI compose). Missing WebCodecs/bytes stay encode-first INCOMPLETE. See [`PLAYABLE_BROWSER_DECODE.md`](PLAYABLE_BROWSER_DECODE.md).  
