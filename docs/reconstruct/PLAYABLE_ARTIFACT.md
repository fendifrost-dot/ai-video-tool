...
**[DECISION]** Lane E2 owns scoring (`src/lib/eval/**`). Lane H calls `evaluateVideoQa` / `videoQaInputFromReconstructE2e` / `videoQaReportToJson` only. Encode-first with `frames:[]` is INCOMPLETE (`awaiting decoded_frames`); `blockingArtifactProducer` is always false. **H owns MP4 decode** (`decodeMp4.ts`, node/ffmpeg). Injected `decodedFrames` score with `frames>0`. Browser Hero Frame Export does not import ffmpeg — live click stays encode-first INCOMPLETE until a browser decoder exists. See [`PLAYABLE_DECODE.md`](PLAYABLE_DECODE.md).  
...
8. Click once. Expect toast `PLAYABLE compose 720×1280 frames=8 … paidCalls=false.` plus Lane E2 `evaluateVideoQa` **INCOMPLETE** (encode-first on the committed 72-frame MP4, `awaiting decoded_frames`, `mp4=produced`, `fail=0`, `blockingArtifactProducer=false`, `stillGoldensReopened=false`). Do **not** expect FAIL — the 8-frame UI window is not decoded MP4 rasters. **Do not expect decoded-frame PASS after this decode-wiring PR's Publish either** — the browser control still does not run ffmpeg. Node/Vitest decode of the committed MP4 is the `frames>0` proof ([`PLAYABLE_DECODE.md`](PLAYABLE_DECODE.md)).
