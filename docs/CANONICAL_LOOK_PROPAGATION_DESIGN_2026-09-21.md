...
## 6. Results of E2 and E1 (2026-09-21, run under the $50 testing budget Fendi set)

**E2 — hero stills via `/images/edits` with an approved anchor** (`grok-image-garment-proxy` after PR #156; 14 stills on S06 scene frames, one anchor = S11 v4c frame 80, `referenceMode: flat`, 1 product ref; $1.68). Finding: the provider takes **at most 3 input images** (`xai:images/edits` capability is now pinned to 3 — sending 5 is a 400, unbilled). The stills DO reproduce the anchor's garment realisation (anchor colour similarity flat across stills, `hero_stills_qa.py`), but the generator **does not hold the pose lock**: arms and hands move relative to the scene frame in most stills, so the still's garment cannot be pasted back by diff mask alone (ghost limbs, propagated heads). `propagate_keyframe.py --hero-stills` therefore intersects the still's changed region with the person silhouette of both frames (RVM) and excludes a geometric head band; that removes the ghosts but leaves the moved-arm frames unusable as heroes. **E2 is a consistency mechanism for appearance, not a hero source for pixel propagation on dance footage.** Kept as a tool (stills + QA + runner), not on the production path.

**E1 — the approved Look-on-artist frame as the Look's `outfit_sheet_path`** ($0.55 per 6.9 s shot; 8 shots + 1 retry ≈ $4.2). Because the deployed video proxy already sends the outfit sheet first with a sentence declaring it the approved realisation, putting the anchor frame there makes `/videos/edits` reproduce **that** realisation across shots: S06/S08/S11/S12 came back with the S11 anchor's construction (stand collar, zipped, band + single wordmark, plain outer sleeves except where the generator insists — S09 crossed-arm shot shows a sleeve stripe on the folded forearms); the pre-hook Look (trucker) anchored on S04 v4b frame 60 held across S01/S03. The S09 first anchored roll produced a hood + vertical placket stripe; a retry with one added negative sentence (`promptVersion v4c-anchor-e1b`) fixed it. Temporal residual on S06 E1 is 13 % below v4b. **E1 is the production mechanism for canonical-Look consistency**: it is data on the Look row (no deploy, no code), it costs one video edit per shot, and every re-roll of any shot inherits the same realisation. Architecture C propagation stays the fallback for pose-locked repairs.

**Deterministic brand layer on top of E1** (`garment_graphic_track.py`, canonical band crop from the product photo): multi-anchor per shot (2–3 measured quads), a shape gate that drops collapsed fits, and the stray-mark eraser (off-colour islands next to the plane inpainted, occluders left alone) now give S06/S08/S09/S11/S12 the same wordmark with the generator's extra glyphs removed. Environment composites use `composite_environment.py --refine shadow`. Assembly of the v6 section is from those files.
