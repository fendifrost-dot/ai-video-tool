...
- **Decision:** Remove the accidental `FOR ALL TO anon, authenticated USING (true)
  ...
  `prop_library`, `artist_looks` and the anon SELECT/INSERT/UPDATE/DELETE policies on
  the `look-composites` storage bucket, and **restore owner-scoped least-privilege
  policies** (`user_id = auth.uid()`, and for `character_features` the `artist_id →
  artists.user_id` join; owner-folder-prefix for the bucket). Authored as revert
  migration `supabase/migrations/20260806120000_risk_001_revert_anon_rls.sql`.
...
- **Status:** Revert migration authored; **NOT applied, NOT merged.** Awaiting Class-C
  architecture + security review, then apply per
