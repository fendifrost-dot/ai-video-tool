...
`repair_method_version` for this wiring is `architecture_c_sleeve_still_1c` (not a chest 1m bump). Hidden shoulder→cuff stays unvalidated. Stage 1a (`architecture_c_sleeve_still_1a`) and 1b (`architecture_c_sleeve_still_1b`) are historical: live FAIL #6 (1a both-side cream; 1b right cream-majority over dark V2 ring). Live 1c `fdb86b18` is **CLEARED 6/6** (navy-ward both sides).
