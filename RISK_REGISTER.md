...
- **Summary:** Isolated `src/lib/reconstruct/playable/**` composes the entire canonical 3.0 s window (72 frames @ 24 fps) at Architecture C working raster **720×1280**. Intended SAM-3 is Stage 1h evidence (`liveFetch=false`; size mismatch fails closed). Full-clip temporal uses in-lib `propagateRepair` (does not edit C2 QA metrics; does not hit the edge 24-frame cap). ffmpeg writes a playable H.264 MP4 with probed claims. After E2 PR #116, H calls `evaluateVideoQa(videoQaInputFromReconstructE2e(e2e, mp4))` and persists `videoQaReportToJson`. Encode-first `frames:[]` is INCOMPLETE (`awaiting decoded_frames`); `blockingArtifactProducer` is always false. Hero Frame §7 **Export playable reconstruct $0** runs an 8-frame window of the same pipeline. No Grok, no new edge, no paint edits.
- **Pointer:** [`docs/reconstruct/PLAYABLE_ARTIFACT.md`](docs/reconstruct/PLAYABLE_ARTIFACT.md); `src/lib/reconstruct/playable/`; issue [#111](https://github.com/fendifrost-dot/ai-video-tool/issues/111).
- **DoD (target):** playable MP4 + claims + E2 hook + `video-qa.json` committed; `npm run reconstruct:playable`; parent Publishes frontend only. **Not claimed:** live decode of storage bytes for master `76fe7438` (row is 1080×1920 HDR); live SAM-3 fetch.
