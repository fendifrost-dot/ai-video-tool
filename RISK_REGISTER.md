...
- **Pointer:** [`docs/reconstruct/PLAYABLE_ARTIFACT.md`](docs/reconstruct/PLAYABLE_ARTIFACT.md); [`docs/reconstruct/PLAYABLE_PORTABILITY.md`](docs/reconstruct/PLAYABLE_PORTABILITY.md); `src/lib/reconstruct/playable/`; issue [#111](https://github.com/fendifrost-dot/ai-video-tool/issues/111); sprint stretch [#102](https://github.com/fendifrost-dot/ai-video-tool/issues/102).
- **DoD (target):** playable MP4 + claims + E2 hook + `video-qa.json` committed; `npm run reconstruct:playable`; parent Publishes frontend only. **Not claimed:** live decode of storage bytes for master `76fe7438` (row is 1080×1920 HDR); live SAM-3 fetch. **2nd-clip stretch:** in-lib catalog bind for `ysl-ice-on-v2-edited-clip` / `f31bd0f2` — live Hero Frame Export on that clip is **NOT CLEARED**.
