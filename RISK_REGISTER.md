...
- **Pointer:** [`docs/temporal/LIVE_SMOKE.md`](docs/temporal/LIVE_SMOKE.md); [`docs/temporal/LIVE_PREP.md`](docs/temporal/LIVE_PREP.md); [`src/lib/heroFrame/temporalDispatch.ts`](src/lib/heroFrame/temporalDispatch.ts); [`src/lib/temporal/livePrep.ts`](src/lib/temporal/livePrep.ts); [`supabase/functions/temporal-propagate-proxy/README.md`](supabase/functions/temporal-propagate-proxy/README.md).
- **DoD (target):** parent Lovable-redeploys **only** `temporal-propagate-proxy` (from #88); Hero Frame flag flip is frontend-only; no still-repair edge redeploy; no per-frame Grok; no proxy-auth widen. **Remaining:** parent computerUse owner-JWT `$0` POST (`./scripts/temporal-live-smoke.sh`) — this VM is BLOCKED on JWT (OPTIONS 200 / anon 401), same as prior still verifies.
