...
- **DoD (target):** playable MP4 + claims + E2 hook + `video-qa.json` committed; `npm run reconstruct:playable`; parent Publishes frontend only. **Live 8-frame click after #133 Publish:** PASS 3/9 `fail=0` `frames=8` `mp4=produced` `browserDecode=webcodecs` liveSample of the 72-frame gate (2026-09-16 ~2:05 AM CT). **In-lib decode:** E2 `frames>0` off committed MP4 bytes (node/ffmpeg or injected rasters). **Not claimed:** full 72-frame live browser decode; live 241-frame/1080 ingest of master `76fe7438` (row is 1080×1920 HDR); live SAM-3 fetch; 2nd-clip live Export; raising `maxFrames`; CLEARED real-media gate. **2nd-clip stretch:** in-lib catalog bind for `ysl-ice-on-v2-edited-clip` / `f31bd0f2` — live Hero Frame Export on that clip is **NOT CLEARED**.
