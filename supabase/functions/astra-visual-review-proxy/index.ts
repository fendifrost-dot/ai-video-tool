...
serve(async (req) => {
  ...
  if (!body.projectId || !body.draftId || !body.instructions || !Array.isArray(body.frames) || !body.jsonSchema?.schema) {
  ...
  if (frames.length === 0) return json(400, { error: "no_frames" });
  ...
  const record = { draftId: body.draftId, partId: body.partId ?? "default", model, reviewedAt: new Date().toISOString(), latencyMs: Date.now() - t0, usage, actualCostUsd, frames: frames.map((f) => f.label), references: refs.map((r) => r.label), review, rawText: review ? undefined : outText.slice(0, 8000) };
  ...
  return json(200, { billed: true, ...plan, actualCostUsd, usage, latencyMs: record.latencyMs, storedPath: upErr ? null : storedPath, storeError: upErr?.message ?? null, review, rawText: review ? undefined : outText.slice(0, 4000) });
});
