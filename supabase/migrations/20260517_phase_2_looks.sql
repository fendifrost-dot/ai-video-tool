...
-- Artist-scoped saved looks (face + wardrobe + jewelry composed into one
-- identity-locked photo), project-level pinning, shot-level lock.
-- Idempotent: safe to re-run.

-- =============================================================================
-- 1. artist_looks — the saved composed look
-- =============================================================================
...
create index if not exists artist_looks_artist_idx
  on public.artist_looks(artist_id);

create index if not exists artist_looks_status_idx
  on public.artist_looks(artist_id, status);

create index if not exists artist_looks_parent_idx
  on public.artist_looks(parent_look_id);
...
-- =============================================================================
-- 2. project_look_picks — project-level pinning of looks
-- =============================================================================
...
create index if not exists project_look_picks_project_idx
  on public.project_look_picks(project_id);

create index if not exists project_look_picks_look_idx
  on public.project_look_picks(look_id);
...
-- =============================================================================
-- 3. shots.locked_look_id — bind a shot to a specific look
-- =============================================================================
alter table public.shots
  add column if not exists locked_look_id uuid
    references public.artist_looks(id) on delete set null;

create index if not exists shots_locked_look_idx
  on public.shots(locked_look_id);

-- =============================================================================
-- 4. look-composites storage bucket — for the rendered look images
-- =============================================================================
...
-- Owner-only access by leading folder = user_id (matches existing pattern).
do $$
declare op text;
begin
