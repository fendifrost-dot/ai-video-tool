-- Phase A — Storyboard schema (UI deferred to Phase B)
-- One storyboard per project, ordered nodes linking to existing shots.
...
create index if not exists storyboard_nodes_storyboard_idx
  on public.storyboard_nodes(storyboard_id, node_order);

create index if not exists storyboard_nodes_shot_idx
  on public.storyboard_nodes(shot_id);

-- Status / strategy / realism / difficulty get DB-level CHECKs so typos can't
-- slip through. We allow nulls for forward compatibility on optional fields.
do $$
begin
  if not exists (select 1 from pg_constraint where conname = 'storyboard_nodes_status_check') then
    alter table public.storyboard_nodes
      add constraint storyboard_nodes_status_check
  ...
  if not exists (select 1 from pg_constraint where conname = 'storyboard_nodes_realism_check') then
    alter table public.storyboard_nodes
      add constraint storyboard_nodes_realism_check
  ...
  if not exists (select 1 from pg_constraint where conname = 'storyboard_nodes_difficulty_check') then
    alter table public.storyboard_nodes
      add constraint storyboard_nodes_difficulty_check
  ...
  if not exists (select 1 from pg_constraint where conname = 'storyboard_nodes_strategy_check') then
    alter table public.storyboard_nodes
      add constraint storyboard_nodes_strategy_check
...
create policy "Users access own storyboards"
...
create policy "Users access own storyboard_nodes"
  on public.storyboard_nodes
  for all
  using (
    storyboard_id in (
      select id from public.storyboards where project_id in (
        select id from public.video_projects where user_id = auth.uid()
      )
    )
  )
