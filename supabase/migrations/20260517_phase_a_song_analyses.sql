-- Phase A — Song Intelligence
-- Analyzed song profile (BPM, beats, energy curve, drops, sections, hooks).
-- One row per project (unique constraint on project_id).
...
create index if not exists song_analyses_project_idx
  on public.song_analyses(project_id);

alter table public.song_analyses enable row level security;

drop policy if exists "Users access own song_analyses"
