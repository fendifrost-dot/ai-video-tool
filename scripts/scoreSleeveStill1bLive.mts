/**
 * Lane B sleeve still Stage 1b live scorer (score-only).
 * ImageScript 1.3.0 for the JPEG source (same decoder as Stage 1a / 1m / edge).
 *
 * Usage:
 *   AVT_SLEEVE_LIVE_DIR=/tmp/avt-sleeve-1b-live npx tsx scripts/scoreSleeveStill1bLive.mts
 */
...
const { Image } = require("/tmp/avt-sleeve-1b-tools/node_modules/imagescript/ImageScript.js") as {
  Image: {
...
const W = 720;
const H = 1280;
const DIR = process.env.AVT_SLEEVE_LIVE_DIR ?? "/tmp/avt-sleeve-1b-live";
const REPO_OUT = process.env.AVT_SLEEVE_REPO_EVIDENCE ?? "docs/sleeve-panel/live-1b";

const LIVE_META = JSON.parse(
  readFileSync(process.env.AVT_SLEEVE_LIVE_META ?? `${DIR}/live_asset.json`, "utf8"),
) as {
  assetId: string;
  created_at: string;
  storedPath: string;
...
const output = await decodeImage(`${DIR}/sleeve_a4dc7f47.png`);
...
const labels = ["clean 2aa1a44c", "LIVE sleeve a4dc7f47"] as const;
