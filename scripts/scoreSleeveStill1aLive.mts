/**
 * Lane B sleeve still live scorer (score-only).
 * ImageScript 1.3.0 for the JPEG source (same decoder as Stage 1m / edge).
 *
 * Usage:
 *   AVT_SLEEVE_LIVE_DIR=/tmp/avt-sleeve-1a-live npx tsx scripts/scoreSleeveStill1aLive.mts
 */
...
const { Image } = require("/tmp/avt-sleeve-1a-tools/node_modules/imagescript/ImageScript.js") as {
  Image: {
...
const W = 720;
const H = 1280;
const DIR = process.env.AVT_SLEEVE_LIVE_DIR ?? "/tmp/avt-sleeve-1a-live";
const REPO_OUT = process.env.AVT_SLEEVE_REPO_EVIDENCE ?? "docs/sleeve-panel/live-1a";
...
const output = await decodeImage(`${DIR}/sleeve_fde270bf.png`);
...
const labels = ["clean 2aa1a44c", "LIVE sleeve fde270bf"] as const;
