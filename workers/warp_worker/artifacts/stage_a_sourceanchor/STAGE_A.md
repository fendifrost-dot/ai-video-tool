...
- Flicker near-anchor = 4.914, whole-window = 6.136.
- Identity drift outside garment = 0.0635 (≈ lossless).
- Edge halo mean = 0.929.
...
| temporal_stability | PASS-near / DECAYS-far | flicker near-anchor=4.91, whole-window=6.14; single-anchor reach=+/-15 frames | Real Kolors garment holds temporally NEAR the anchor; degrades with distance because only ONE real anchor exists — that decay IS the re-anchor cadence answer. |
...
| artifact_severity | PASS-near / WARN-far | edge_halo mean=0.93, id_drift_outside=0.063 | Composite outside garment stays lossless; far-from-anchor warps stretch occluded Kolors regions (single-anchor holes) — expected, flagged by the reach gate. |
